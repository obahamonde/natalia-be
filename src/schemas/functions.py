...
class NginxDocker(FunctionType,AbstractDocker):
    ...
    def payload(self):
        return {
            "Image": self.image,
            "HostConfig": {
                "PortBindings": {f"{self.port}/tcp": [{"HostPort": str(self.host_port)}]}
            },
        }
    ...
    async def run(self):
        """Run the function"""
        response = await docker.fetch("/containers/create", method="POST", json=self.payload())
        id_ = response["Id"]
        await docker.text(f"/containers/{id_}/start", method="POST")
        await asyncio.sleep(1)
        response = await docker.fetch(f"/containers/{id_}/json", method="GET")
        dns_response = await cf.provision(f"{self.image}-{self.host_port}",self.host_port)
