...
from aiofauna.openai import (Field, FunctionType, chat_completion,
                             function_call, lang_chain, make_client)
...
worldvectorlogos = make_client(base_url="https://worldvectorlogo.com", headers={"User-Agent": "Mozilla/5.0 (X11; Linux x86_64; rv:89.0) Gecko/20100101 Firefox/89.0"})
...
class SearchLogo(FunctionType):
    ...
    async def run(self):
        text = await worldvectorlogos.text(
            f"/search/{self.query}"
        )
        soup = BeautifulSoup(text, "html.parser")
